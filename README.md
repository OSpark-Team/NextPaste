...
├── server/               # Windows/Mac/Linux 桌面端 (Wails: Go + Vue3)
│   ├── frontend/         # 桌面端管理界面 (Vue3 + TS)
│   ├── internal/         # Go 后端逻辑
│   │   ├── websocket/    # WebSocket 服务器和客户端
│   │   ├── clipboard/    # 剪贴板监听
│   │   └── protocol/     # 通信协议
│   └── main.go           # Wails 入口
├── relay-server/         # 中继服务器 (纯 Go 命令行)
│   ├── main.go           # 服务器入口
│   ├── relay.go          # 中继逻辑
│   └── README.md         # 中继服务器文档
├── docs/                 # 项目文档及通信协议
...
* **双模式运行**：支持服务器模式和客户端模式
  - **服务器模式**：在局域网内提供 WebSocket 服务，其他设备连接到本机
  - **客户端模式**：连接到远程 WebSocket 服务器（包括中继服务器）
...
* **纯转发**：不处理剪贴板，只负责消息转发，保护隐私
* **无限房间**：支持无限数量的房间，自动创建和清理
* **跨网络同步**：解决局域网限制，通过公网中继实现跨网络剪贴板共享
* **轻量高效**：纯命令行，资源占用极低，支持数千并发连接
...
* **通信协议**：默认为 **V1.1 二进制协议** (高效、支持分片传输，在1.2后支持V2二进制帧模式，并且不兼容V1.0)。
